...
export default function DiscoverRow({ title, items }: { title: string; items: StremioMeta[] }) {
  ...
  return (
    <section className="group/row relative mb-11 pl-6 md:pl-12 lg:pl-16">
      <div className="mb-4 flex items-end gap-3">
        ...
        <span className="mb-0.5 text-[10px] font-bold uppercase tracking-[0.17em] text-zinc-600">Curated for you</span>
      </div>
      <button
        onClick={() => scroll(-1)}
      ...
      <div ref={rowRef} className="no-scrollbar flex gap-4 overflow-x-auto pb-5 pr-6 md:pr-12 lg:pr-16">
        {items.map((item) => <DiscoverCard key={`${item.type}:${item.id}`} item={item} />)}
      </div>
      <button
        onClick={() => scroll(1)}
